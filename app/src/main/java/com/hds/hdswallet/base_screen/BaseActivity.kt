...
import com.hds.hdswallet.core.views.HdsToolbar
import com.hds.hdswallet.screens.app_activity.AppActivity
import java.util.*
import com.hds.hdswallet.screens.welcome_screen.welcome_open.WelcomeOpenFragment
...
abstract class BaseActivity<T : BasePresenter<out MvpView, out MvpRepository>> : ViewControllerAppCompatActivity(), MvpView, ScreenDelegate.ViewDelegate {
    ...
    override fun initToolbar(title: String?, hasBackArrow: Boolean?, hasStatus: Boolean) {
        val toolbarLayout = this.findViewById<HdsToolbar>(R.id.toolbarLayout) ?: return
        setupToolbar(toolbarLayout, title, hasBackArrow, hasStatus)
    }

    fun setupToolbar(toolbar: HdsToolbar?,title: String?, hasBackArrow: Boolean?, hasStatus: Boolean) {
        if (toolbar == null) {
            return
        }
...
//            finish()
    ...
    private fun handleStatus(isOnline: Boolean, toolbarLayout: HdsToolbar) {
