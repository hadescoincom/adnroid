 ...
 */
object QrHelper {
    const val HDS_URI_PREFIX = "hds://"
    private const val HDS_QR_PREFIX = "hds:"
    ...
    fun isNewQrVersion(text: String) = text.startsWith(HDS_QR_PREFIX)
    ...
    fun getScannedAddress(text: String): String {
        return if (text.startsWith(HDS_QR_PREFIX)) {
            val removePrefix = text.removePrefix(HDS_QR_PREFIX)
    ...
    fun createQrString(receiveToken: String, amount: Double?) = "$HDS_QR_PREFIX$receiveToken" +
            if (amount != null) "?$AMOUNT_PARAMETER=${amountDecimalFormat.format(amount).replace(" ", "")}" else ""

    fun parseQrCode(text: String): QrObject {
        val uri = Uri.parse(text.replace(HDS_QR_PREFIX, HDS_URI_PREFIX))
