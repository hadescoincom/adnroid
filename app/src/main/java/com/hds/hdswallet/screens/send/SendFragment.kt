...
import com.hds.hdswallet.core.watchers.AmountFilter
import com.hds.hdswallet.core.watchers.InputFilterMinMax
import com.hds.hdswallet.core.watchers.OnItemSelectedListener
import com.hds.hdswallet.core.watchers.TextWatcher
import com.hds.hdswallet.screens.addresses.AddressPagerType
import com.hds.hdswallet.screens.addresses.AddressesAdapter
import com.hds.hdswallet.screens.addresses.AddressesPagerAdapter
import com.hds.hdswallet.screens.addresses.Tab
import com.hds.hdswallet.screens.change_address.ChangeAddressCallback
import com.hds.hdswallet.screens.change_address.ChangeAddressFragment
...
class SendFragment : BaseFragment<SendPresenter>(), SendContract.View {
    ...
    override fun showFeeDialog() {
        ...
        view.findViewById<HdsButton>(R.id.btnSave).setOnClickListener {
            val rawFee = feeEditText.text?.toString()
            val fee = rawFee?.toLongOrNull() ?: 0
            if (fee >= minFee) {
        ...
        return try {
            when {
                ...
                amount + fee > availableAmount -> {
                    configAmountError(configAmountErrorMessage(((availableAmount - (amount + fee)) * -1).convertToHdsString(), isEnablePrivacyMode))
                    true
                }
                else -> false
            }
        } catch (exception: NumberFormatException) {
            configAmountError(configAmountErrorMessage(amount.convertToHdsString(), isEnablePrivacyMode))
            true
        }
    }
    ...
    override fun setAmount(amount: Double) {
        this.amount.setText(amount.convertToHdsString())
...
//            addressContainer?.setBackgroundColor(ContextCompat.getColor(context!!, colorId))
    ...
    override fun updateAvailable(available: Long) {
        ...
        availableSum.text = "${available.convertToHdsString()} ${getString(R.string.currency_hds).toUpperCase()}"
