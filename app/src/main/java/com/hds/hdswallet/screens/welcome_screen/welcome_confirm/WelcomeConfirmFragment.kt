...
class WelcomeConfirmFragment : BaseFragment<WelcomeConfirmPresenter>(), WelcomeConfirmContract.View {
    ...
    private fun isSeedValid(): Boolean {
        for (i in 0 until seedLayout.childCount) {
            if (!(seedLayout.getChildAt(i) as HdsPhraseInput).isValid) {
                return false
            }
        }
    ...
    private fun onFocusNextField() {
        for (i in 0 until seedLayout.childCount) {
            val phrase = (seedLayout.getChildAt(i) as HdsPhraseInput)
            if (!phrase.isValid) {
    ...
    override fun configSeed(seedToValidate: List<Int>, seed: Array<String>) {
        ...
        (seedLayout.getChildAt(seedToValidate.size - 1) as HdsPhraseInput).editText.imeOptions = EditorInfo.IME_ACTION_DONE
