...
import com.hds.hdswallet.screens.transaction_details.TransactionDetailsFragmentArgs
...
import com.hds.hdswallet.screens.wallet.NavItem
import com.hds.hdswallet.screens.wallet.NavItemsAdapter
...
import com.hds.hdswallet.base_screen.*
import com.hds.hdswallet.core.entities.NotificationItem
import com.hds.hdswallet.core.entities.NotificationType
import com.hds.hdswallet.core.views.NotificationBanner
import com.hds.hdswallet.screens.address_details.AddressFragment
import com.hds.hdswallet.screens.address_details.AddressFragmentArgs
import com.hds.hdswallet.screens.notifications.NotificationsFragment
import com.hds.hdswallet.screens.notifications.newversion.NewVersionFragment
import com.hds.hdswallet.screens.notifications.newversion.NewVersionFragmentArgs
import com.hds.hdswallet.screens.qr.ScanQrActivity
import com.hds.hdswallet.screens.transaction_details.TransactionDetailsFragment
...
class AppActivity : BaseActivity<AppActivityPresenter>(), AppActivityContract.View {
    ...
    companion object {
        ...
        const val BUY_ID = "android.intent.action.BUY_HDS"
        const val RECEIVE_ID = "android.intent.action.RECEIVE_HDS"
        const val SEND_ID = "android.intent.action.SEND_HDS"
    ...
    fun checkShortCut() {
        if(App.isAuthenticated && shortCut!=null)
        {
            if(shortCut == BUY_ID) {
                ...
                if (allow) {
                    openExternalLink(AppConfig.HDS_EXCHANGES_LINK)
                }
                else{
                    showAlert(
    ...
    private fun setupMenu(savedInstanceState: Bundle?)
    {
       menuItems =  mutableListOf(
        ...
        val whereBuyHdsLink = navigationView.findViewById<TextView>(R.id.whereBuyHdsLink)
        whereBuyHdsLink.setOnClickListener {
    ...
    fun reloadMenu() {
        val whereBuyHdsLink = navigationView.findViewById<TextView>(R.id.whereBuyHdsLink)
        whereBuyHdsLink.text = getString(R.string.welcome_where_to_buy_hds)
    ...
    private fun sendNotifications() {
        ...
        if (count > 1 && AppManager.instance.allUnsentIsAddresses()) {
            ...
            val view = self.findViewById<View>(android.R.id.content)
            val banner = NotificationBanner.make(view, self, title, spannableString, R.drawable.ic_icon_hds_new, NOTIFICATION_ID_ALL,"", NotificationType.News) { notificationId, objectId, type ->
                openNotification(notificationId, objectId, type)
            }
            banner.show()
        }
